...
export class UsersModule extends ModuleBase {
	...
	getColorForUsername(username) {
		let hash = _.reduce(username, (hash, ch) =>
			ch.charCodeAt(0) + (hash << 6) + (hash << 16) - hash, 0);
		hash = Math.abs(hash);
		const hue = hash % 360;
	...
	registerClient(client) {
		client.onActions({
			...
			"auth:login": ({name}) => { return this.loginClient$(client, name); },
			"auth:logout": () => { this.logoutClient(client); }
		});

		client.on("disconnect", () => { this.logoutClient(client); });
	}
}